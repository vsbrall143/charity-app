...
if (loginForm) {
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/auth/charitylogin', { email, password }); 
      console.log(res);
...
if (registerForm) {
  registerForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/auth/charityregister', { name, email, password, mission,registrationNumber });
