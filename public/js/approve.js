...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const response = await axios.get('http://localhost:5000/unapprovedcharities');
  ...
  async function loadProjects() {
    ...
    try {
      const response = await axios.get('http://localhost:5000/unapprovedprojects');
  ...
  async function approveCharity(charityId) {
    try {
      ...
      await axios.post(`http://localhost:5000/approvecharity/${charityId}`,   {  headers: { Authorization: token } });
