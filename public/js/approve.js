...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const response = await axios.get('https://charity-app-5acl.onrender.com/unapprovedcharities');
  ...
  async function loadProjects() {
    ...
    try {
      const response = await axios.get('https://charity-app-5acl.onrender.com/unapprovedprojects');
  ...
  async function approveCharity(charityId) {
    try {
      ...
      await axios.post(`https://charity-app-5acl.onrender.com/approvecharity/${charityId}`,   {  headers: { Authorization: token } });
